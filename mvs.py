...
def init_log():
    ...
    # log 출력
    logfilePath = os.path.join(cwd, f'sys_log/{log_time}')
    print(logfilePath)
    sys_log_handler = logging.FileHandler(logfilePath)
    sys_log_handler.setFormatter(
        logging.Formatter('%(levelname)s - %(message)s'))
    sys_logger.addHandler(sys_log_handler)


def run(vehicle_num: int, task_num: int):
    ...
    reqfilePath = os.path.join(cwd, f'data/{request_name}')
    df_task = pd.read_csv(reqfilePath)
    ...
    logfilePath = os.path.join(cwd, f'../log/{request_date}_{simulation_vehicle_num}_{simulation_task_num}.json')
    print(f"write log : {logfilePath}")
